...
onPageLoad((signal) => {
  ...
  if (root) {
    const copy: PortfolioContent["openSource"]["github"] = JSON.parse(
      root.querySelector("[data-github-copy]")!.textContent!,
    );
    let snapshot = validateGitHubSnapshot(
      JSON.parse(root.querySelector("[data-github-snapshot]")!.textContent!),
    );
    const calendar = initGitHubCalendar(
      root.querySelector<HTMLElement>("[data-github-calendar]")!,
    );
    ...
    const syncFormat = new Intl.DateTimeFormat(locale, {
      dateStyle: "medium",
      timeStyle: "short",
      timeZone: "UTC",
    });
    ...
    const setStatus = (message: string): void => {
      const stale =
        Date.now() - Date.parse(snapshot.updatedAt) > 36 * cacheLifetime;
      status.textContent = [message, stale ? copy.staleLabel : ""]
        .filter(Boolean)
        .join(" ");
    };
    ...
    const renderRequests = (): void => {
      ...
      const rows = snapshot.pullRequests[scope].map((request) => {
        ...
        const repository = row.querySelector<HTMLAnchorElement>(
          "[data-pr-repository]",
        )!;
        repository.textContent = request.repository;
        repository.href = `https://github.com/${request.repository}`;
        row.querySelector("[data-pr-number]")!.textContent =
          `#${request.number}`;
        ...
        row.querySelector("[data-pr-relationship]")!.textContent =
          copy.relationships[request.relationship];
        const title = row.querySelector<HTMLAnchorElement>("[data-pr-title]")!;
        title.textContent = request.title;
        title.href = request.url;
        const created =
          row.querySelector<HTMLTimeElement>("[data-pr-created]")!;
        ...
        row.querySelector("[data-pr-event-label]")!.textContent =
          request.mergedAt ? copy.mergedLabel : copy.updatedLabel;
        row.querySelector<HTMLAnchorElement>("[data-pr-discussion]")!.href =
          request.url;
        row.querySelector<HTMLAnchorElement>("[data-pr-files]")!.href =
          `${request.url}/files`;
        row.querySelector<HTMLAnchorElement>("[data-pr-commits]")!.href =
          `${request.url}/commits`;
        return row;
      });
      ...
      for (const button of buttons)
        button.setAttribute(
          "aria-pressed",
          String(button.dataset.githubScope === scope),
        );
      if (previousLink) {
        const replacement = [
          ...list.querySelectorAll<HTMLAnchorElement>("a"),
        ].find((link) => link.getAttribute("href") === previousLink);
        (
          replacement ??
          buttons.find((button) => button.dataset.githubScope === scope)
    ...
    const applySnapshot = (next: GitHubSnapshot): void => {
      ...
      renderRequests();
      if (!preview) {
        root.querySelector("[data-github-open]")!.textContent = String(
          snapshot.counts.open,
        );
        root.querySelector("[data-github-merged]")!.textContent = String(
    ...
    const synchronize = async (): Promise<void> => {
      ...
      try {
        ...
        if (!response.ok)
          throw new Error(`GitHub snapshot: HTTP ${response.status}`);
        ...
        try {
          localStorage.setItem(
            cacheKey,
            JSON.stringify({ checkedAt: Date.now(), snapshot }),
          );
        } catch {
    ...
    if (!preview)
      root.querySelector<HTMLElement>("[data-github-scopes]")!.hidden = false;
