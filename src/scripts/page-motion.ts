...
if (!reducedMotion.matches && "IntersectionObserver" in window) {
  ...
  const selector = ".section-shell > *, [data-motion-reveal], #projects > h2, #projects article, .journey-entry, .contact-inner > *";
  ...
  const pendingStyles = new Map<HTMLElement, {
    opacity: string;
    filter: string;
    transform: string;
    willChange: string;
